...
/*   Updated: 2026/01/21 12:40:38 by didris           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_ver_line_height(t_raycast *ray, t_wall_line *ver_line)
{
	...
	ver_line->line_h = (int)(SCREEN_H / ray->perp_w_dist);
	ver_line->draw_start = -ver_line->line_h / 2 + SCREEN_H / 2;
	ver_line->draw_end = ver_line->line_h / 2 + SCREEN_H / 2;
	if (ver_line->draw_start < 0)
		ver_line->draw_start = 0;
	if (ver_line->draw_end >= SCREEN_H)
		ver_line->draw_end = SCREEN_H - 1;
}
...
void	draw_wall_tex(t_wall_tex *wall, t_wall_line *line, t_image *img, int x)
{
	...
	wall->tex_pos = (line->draw_start - SCREEN_H / 2 + line->line_h / 2) * step;
	y = line->draw_start;
