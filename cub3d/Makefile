...
SRCS =  main.c	   \
		...
		parser/parsing_val.c	  \
		parser/parsing.c		  \
		executer/draw.c	\
		executer/execute.c	\
		executer/execute_utils.c	\
		executer/game.c	\
		executer/raycasting.c\
		minimap.c	\
...
LDFLAGS = $(MLX) -L/usr/lib -lXext -lX11 -lm -lz

all: $(NAME)

$(NAME): $(OBJS) $(GET_NEXT_LINE_OBJ)
	$(CC) $(OBJS) $(GET_NEXT_LINE_OBJ) $(LDFLAGS) -o $(NAME)

%.o: %.c
	$(CC) $(CFLAGS) -c $< -o $@

clean:
	rm -f $(OBJS) $(GET_NEXT_LINE_OBJ)
